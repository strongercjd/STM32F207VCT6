...
/**
  * @brief  从指定扇区位置读取数据.
  * @param  buf: 数据指针.
  * @param  sector: 读取数据的起始扇区.
  * @param  sec_num: 读取的扇区个数.
  * @retval None
*/
uint8_t SD_ReadDisk(uint8_t*buf,uint32_t sector,uint32_t sec_num)
{
  uint8_t sta=SD_OK;
  uint32_t n;
  uint32_t addr;
  ...
  if((uint32_t)buf%4!=0)
  {
    for(n=0;n<sec_num;n++)
    {
      sta=SD_ReadBlock(SDIO_DATA_BUFFER,addr+512*n,512);//单个sector的读操作
  ...
  {
    if(sec_num==1)
      sta=SD_ReadBlock(buf,addr,512);    	//单个sector的读操作
    else 
      sta=SD_ReadMultiBlocks(buf,addr,512,sec_num);//多个sector  
    /* Check if the Transfer is finished */
    Status = SD_WaitReadOperation();
    while(SD_GetStatus() != SD_TRANSFER_OK);
  }
...
/**
  * @brief  从指定扇区位置写入数据.
  * @param  buf: 数据指针.
  * @param  sector: 写入数据的起始扇区.
  * @param  sec_num: 写入的扇区个数.
  * @retval None
  */
uint8_t SD_WriteDisk(uint8_t*buf,uint32_t sector,uint32_t sec_num)
{
  uint8_t sta=SD_OK;
  uint32_t n;
  uint32_t addr;
  ...
  if((uint32_t)buf%4!=0)
  {
    for(n=0;n<sec_num;n++)
    {
      ...
      sta=SD_WriteBlock(SDIO_DATA_BUFFER,addr+512*n,512);//单个sector的写操作
      buf+=512;
      /* Check if the Transfer is finished */
      Status = SD_WaitWriteOperation();
      while(SD_GetStatus() != SD_TRANSFER_OK);
  ...
  {
    if(sec_num==1)
      sta=SD_WriteBlock(buf,addr,512);    	//单个sector的写操作
    else 
      sta=SD_WriteMultiBlocks(buf,addr,512,sec_num);	//多个sector  
    /* Check if the Transfer is finished */
    Status = SD_WaitWriteOperation();
    while(SD_GetStatus() != SD_TRANSFER_OK);
  }
