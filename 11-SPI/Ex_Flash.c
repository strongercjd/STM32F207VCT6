...
* Descriptions        向外部Flash读出数据
* @param 	temp_addr   读数据的地址
* @param 	data_cach   读数据的数据指针
* @param 	data_len    读数据的数据长度
* @return	NONE
* @note
*/
Flash_enum ExFlash_RD(uint32_t temp_addr,uint8_t *data_cach,uint32_t data_len)
{
  if(temp_addr<EX_Flash.Flash_Memory_value){
    Ex_Flash_RD(temp_addr,data_cach,data_len);  
    return (Ex_Flash_OK);
  }else{
    return (Ex_Flash_ERR);
  }
...
/*!
* Function name       ExFlash_WR_NByte
* Descriptions        向外部Flash写入数据
* @param 	temp_addr   写入的地址
* @param 	data_cach   写入的数据指针
* @param 	data_len    写入的数据长度
* @return	NONE
* @note
*/
...
{ 
  if(temp_addr<EX_Flash.Flash_Memory_value){
    Ex_FLASH_Write(temp_addr,data_cach,data_len);
    return (Ex_Flash_OK);
  }else{
    return (Ex_Flash_ERR);
  }
...
/*!
* Function name       Erase_One_Sector
* Descriptions        擦除外部Flash1个扇区
* @param 	Dst   需要擦除的扇区
* @return	NONE
* @note
*/
Flash_enum ExFlash_Erase_Sec(uint32_t Dst)
{
  ...
  SecnHdAddr = Ex_FLASH_SecSize * Dst;		// 计算扇区的起始地址
  
  if(SecnHdAddr<EX_Flash.Flash_Memory_value){
    Ex_FLASH_SectorErase(Dst);
    return (Ex_Flash_OK);
  }else{
    return (Ex_Flash_ERR);
  }
