...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  ...
  if(G_change.sys_enet_init_state == ETH_CONFIG){
    ...
    G_change.sys_enet_init_state = ETH_PHY_RST;
    
    return ETH_SUCCESS;
  }
  /*-------------------- PHY initialization and configuration ----------------*/
  if(G_change.sys_enet_init_state == ETH_PHY_RST){
    ...
    G_change.sys_enet_init_state = ETH_PHY_LINK;
    return ETH_SUCCESS;
  }
  ...
  if(G_change.sys_enet_init_state == ETH_PHY_LINK){
    if(!(ETH_ReadPHYRegister(PHYAddress, PHY_BSR) & PHY_Linked_Status)){
      if((G_timeTicks - G_change.sys_phy_config_time) >= PHY_CONFIG_SEC){
        G_change.sys_enet_init_state = ETH_PHY_LINK;
        G_change.sys_phy_config_time = G_timeTicks;
      }
      /* Return ERROR in case of timeout */
      return ETH_ERROR; 
    }
    G_change.sys_phy_config_time = G_timeTicks;
    G_change.sys_enet_init_state = ETH_PHY_AUTO;
    return ETH_SUCCESS;
  }

  /*使能自动协商机制*/
  /* Enable Auto-Negotiation */
  if(G_change.sys_enet_init_state == ETH_PHY_AUTO){
    ...
    G_change.sys_phy_config_time = G_timeTicks;
    G_change.sys_enet_init_state = ETH_PHY_SET;
    return ETH_SUCCESS;
  }
  
    /* Wait until the auto-negotiation will be completed */
    /*检查自动协商结果*/
  if(G_change.sys_enet_init_state == ETH_PHY_SET){
    ...
    /*根据前面的协商机制，来完成当前网线的状态指示，如网速和双工工作方式*/
    ...
    if((RegValue & 0X100) != (uint32_t)RESET)
    {
  ...
  ETH->DMABMR = (uint32_t)(ETH_InitStruct->ETH_AddressAlignedBeats | 
    ...
    G_change.sys_phy_config_time = G_timeTicks;
    G_change.sys_enet_init_state = ETH_LWIP_INIT;
