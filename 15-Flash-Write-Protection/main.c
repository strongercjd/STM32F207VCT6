...
int main(void)
{
  ...
  if(GPIO_ReadInputDataBit(GPIOE,GPIO_Pin_14) == 0){
    ...
    LCD_ShowString(0,line*16,"Press key 1 for Flash Write Protection test");
    line ++;
    ...
    {
      LCD_ShowString(0,line*16,"Flash are write protected, disable the write protection");
      line ++;
      ...
      {
        LCD_ShowString(0,line*16,"disable the write protection OK");
        line ++;
      }
      else
      {
        LCD_ShowString(0,line*16,"disable the write protection has problems");
        line ++;
      }
    }
    else
    { 
      LCD_ShowString(0,line*16,"Flash are not write protected, enable the write protection");
      line ++;
      /* Enable the Flash option control register access */
      FLASH_OB_Unlock();
      ...
      if (SectorsWRPStatus == 0x00)
      {
        LCD_ShowString(0,line*16,"enable the write protection OK");
        line ++;
      }
      else
      {
        LCD_ShowString(0,line*16,"enable the write protection has problems");
        line ++; 
      }
    }
  }else{
    LCD_ShowString(0,line*16,"Key 1 is not pressed, no Flash Write Protection test");
    line ++;
  }
